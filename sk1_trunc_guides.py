...
set_index_file('brar_data/sk1_trunc_index/sk1_t')
set_chrom_dict(get_chrom_dict(fname='brar_data/sk1_truncations.fasta'))

#sorf_columns = ["chr","start","end","name","score","strand","thickstart","thickend","rgb","exons","sizes","starts"]
sorfs = pd.read_csv('brar_data/sorfs_trunc.bed', delimiter='\t')
sorfs = sorfs.rename(columns={'chrom':'chr','orfname':'name', 'stop':'end'})
print(sorfs.columns)
...
def db_genes_from_bed(db_bed):
    ...
    #assert(np.all(db_bed.starts==0)) #can uses sizes/starts if not, but for now limit to no introns
...
print('removing the following invalid orfs:')
print(sorfs[~sorfs.valid_seq])
sorfs = sorfs[sorfs.valid_seq]
...
print("number of gene with xstart codons", collections.Counter(sorfs.num_starts))
...
print('number of genes with N to remove', sum(sorfs.num_n != 0), 'shown below')
...
db_sorfs.to_csv('brar_data/db_sorfs_truncated.csv', sep='\t')
